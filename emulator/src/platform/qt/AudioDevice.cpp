...
void AudioDevice::setFormat(const QAudioFormat& format) {
	...
	blip_set_rates(m_context->core->getAudioChannel(m_context->core, 0, NULL),
		           m_context->core->frequency(m_context->core), format.sampleRate() * fauxClock);
	blip_set_rates(m_context->core->getAudioChannel(m_context->core, 1, NULL),
		           m_context->core->frequency(m_context->core), format.sampleRate() * fauxClock);
...
qint64 AudioDevice::readData(char* data, qint64 maxSize) {
	...
	mCoreSyncLockAudio(&m_context->impl->sync);
	int available = blip_samples_avail(m_context->core->getAudioChannel(m_context->core, 0, NULL));
	if (available > maxSize / sizeof(GBAStereoSample)) {
		available = maxSize / sizeof(GBAStereoSample);
	}
	blip_read_samples(m_context->core->getAudioChannel(m_context->core, 0, NULL), &reinterpret_cast<GBAStereoSample*>(data)->left, available, true);
	blip_read_samples(m_context->core->getAudioChannel(m_context->core, 1, NULL), &reinterpret_cast<GBAStereoSample*>(data)->right, available, true);
